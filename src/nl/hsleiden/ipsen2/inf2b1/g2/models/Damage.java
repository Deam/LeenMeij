...
public class Damage extends Database
{
	...
	public void Insert(Damage damage)
	{
		try {
			...
			PreparedStatement statement = getConnection().prepareStatement("INSERT INTO damage(description, date, vehicleid) VALUES(?, ?, ?)");
	...
	public void Update(Damage damage, int id)
	{
		try {
			...
			PreparedStatement statement = getConnection().prepareStatement("UPDATE damage SET description = ?, date = ? WHERE damageid = ?");
	...
	public ArrayList<Damage> getDamage(int id){
		...
		try {
			...
			PreparedStatement statement = getConnection().prepareStatement("SELECT * FROM damage WHERE vehicleid = ?");
	...
	public Damage getById(int id)
	{
		...
		try {
			...
			set.next();
			// Customer id is the selected id, so no need to get it from the database
