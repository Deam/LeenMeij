 ...
 * Database connections for inserting, updating, deleting and selecting
 * 
 * @author David
 */
...
public class Financial extends Database
{
	...
	public void Insert(Financial financial)
	{
		try {
			...
			PreparedStatement statement = getConnection().prepareStatement("INSERT INTO financials(rentalid, renteddate, customerid, firstname, lastname, vehicleid, vehiclebrand, vehiclemodel, licenceplate,rentalkost) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
			...
			statement.setString(3, Integer.toString(financial.getCustomerNumber()));
			...
			statement.setString(10, Integer.toString(financial.getRentalKost()));
	...
	public void Update(Financial financial, int id)
	{
		try {
			...
			PreparedStatement statement = getConnection().prepareStatement("UPDATE financials SET rentalid = ?, renteddate = ?, customerid = ?, firstname = ?, lastname = ?, vehicleid = ?, vehiclbrand = ?, vehiclemodel = ?, licenceplate = ?,rentalkost = ? WHERE financialid = ?");
	...
	public void Delete(int id)
	{
		try {
			...
			PreparedStatement statement = getConnection().prepareStatement("DELETE FROM financials WHERE financialid = ?");
	...
	public ArrayList<Financial> getAll_by_Date(String date){
		...
		try {
			...
			// Create the query and execute it
			PreparedStatement statement = getConnection().prepareStatement("SELECT * FROM financials WHERE rentedDate LIKE '%" + date +"%'");
	...
	/** 
	 * Get by id method, for getting a single financial defined by the financialid
	 * @param id
	 * @return
	 */
	public Financial getById(int id)
	{
		...
		try {
			PreparedStatement statement = getConnection().prepareStatement("SELECT * FROM financials WHERE financialid = ?");
			statement.setInt(1, id);
			...
			set.next();
			// Financial id is the selected id, so no need to get it from the database
			f.setFinancialID(id);
