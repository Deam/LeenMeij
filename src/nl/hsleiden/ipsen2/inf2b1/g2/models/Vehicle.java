...
public class Vehicle extends Database{
	...
	public boolean Insert(Vehicle vehicle)
	{
		try {
			...
			PreparedStatement statement = getConnection().prepareStatement("INSERT INTO vehicle(category, brand, model, color, milage, lisence, options, comment, image, available, price_pd) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
	...
	public void Update(Vehicle vehicle, int id)
	{
		try {
			...
			PreparedStatement statement = getConnection().prepareStatement("UPDATE vehicle SET category = ?, brand = ?, model = ?, color = ?, milage = ?, lisence = ?, options = ?, comment =?, image = ?, price_pd = ?, available = ? WHERE vehicleid = ?");
	...
	public void Delete(int id)
	{
		try {
			...
			PreparedStatement statement = getConnection().prepareStatement("DELETE FROM vehicle WHERE vehicleid = ?");
	...
	public ArrayList<Vehicle> getAll(){
		...
		try {
			...
			PreparedStatement statement = getConnection().prepareStatement("SELECT * FROM vehicle");
	...
	public Vehicle getById(int id)
	{
		...
		try {
			...
			connect();
			PreparedStatement statement = getConnection().prepareStatement("SELECT * FROM vehicle WHERE vehicleid = ?");
			statement.setInt(1, id);
			...
			set.next();
			// Vehicle id is the selected id, so no need to get it from the database
			v.setVehicleID(id);
	...
	public void setVehicleAvailable(int vehicleid, int available)
	{
		...
		try {
			// Make the statement
			PreparedStatement statement = getConnection().prepareStatement("UPDATE vehicle SET available = ? WHERE vehicleid = ?");
