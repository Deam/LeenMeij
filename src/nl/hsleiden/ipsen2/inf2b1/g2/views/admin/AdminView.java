...
public class AdminView extends JFrame implements Observer {
	...
	public JMenuItem closeAdmin, addCustomer, addVehicle, addUser,
			customerOverview, vehicleOverview, financialOverview, userOverview;
	...
	public AdminView(ActionListener al, AdminController controller) {
		...
		customerOverviewPanel.setBorder(BorderFactory
				.createTitledBorder("Laatste 50 klanten"));

		customerOverviewPanel.add(
				new JScrollPane(customerTable = controller
						.CustomerTableLimited()), BorderLayout.CENTER);

		//
		vehicleOverviewPanel = new JPanel();
		vehicleOverviewPanel.setLayout(new BorderLayout(0, 0));
		vehicleOverviewPanel.setBorder(BorderFactory
				.createTitledBorder("Alle voertuigen"));

		vehicleController = new VehicleController(this);
		vehicleOverviewPanel.add(new JScrollPane(
				vehicleTable = vehicleController.VehicleTable()),
				BorderLayout.CENTER);

		financialOverviewPanel = new JPanel();
		financialOverviewPanel.setLayout(new BorderLayout(0, 0));
		financialOverviewPanel.setBorder(BorderFactory
				.createTitledBorder("Financieel overzicht"));

		fController = new FinancialController();
		financialOverviewPanel.add(
				new JScrollPane(financialTable = fController.FinancialTable()),
				BorderLayout.CENTER);
				...
				.setVerticalGroup(gl_contentPane
						...
						.addGroup(
								gl_contentPane
										...
										.addComponent(financialOverviewPanel,
												GroupLayout.PREFERRED_SIZE,
	...
	public void update(int message) {
		customerTable.setModel(controller.CustomerTableLimited().getModel());
		vehicleTable.setModel(vehicleController.VehicleTable().getModel());
		financialTable.setModel(fController.FinancialTable().getModel());
	}
